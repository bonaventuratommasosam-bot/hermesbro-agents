#!/usr/bin/env python3
"""Load/save El-froggo Lead config."""
...
DEFAULTS: dict[str, Any] = {
    "org": {"name": "", "founder_chat_id": "", "timezone": "Europe/Rome"},
    "fleet": {"report_time": "08:00", "include_trading": False, "agents": []},
    "telegram": {"group_chat_id": "", "admin_chat_id": ""},
    "roles": {"admin": []},
    "reports": {"daily_enabled": False, "last_brief": ""},
    "cron": {"daily_brief": "0 8 * * *", "enabled": False},
    "language": {"default": "it"},
}

SQUAD = ["contabile", "designbro", "ducato", "frank", "groot", "lawrenzo", "machiavelli", "sentinel", "wannabe"]
...
def config_path(root: Path | None = None) -> Path:
    return (root or profile_root()) / "fleet-lead-config.yaml"
...
def lead_context(cfg: dict | None = None) -> dict:
    c = cfg or load()
    agents = get_dotted(c, "fleet.agents") or SQUAD
    return {
        "org": (get_dotted(c, "org.name") or "HermesBro").strip(),
        "agents": agents,
        "report_time": get_dotted(c, "fleet.report_time") or "08:00",
        "founder": get_dotted(c, "org.founder_chat_id") or "",
    }
