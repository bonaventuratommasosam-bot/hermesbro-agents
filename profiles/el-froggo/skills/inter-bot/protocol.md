...
All bots are Hermes profiles sharing one Telegram supergroup (chat <GROUP_CHAT_ID>). Each bot has its own topic/thread. Cross-bot messaging = send message to another bot's topic.

## Bot Registry

Registry file: `<HERMES_SHARED>/registry/bots.json`

| Bot | Profile | Thread ID | Role |
|-----|---------|-----------|------|
| gribbito | gribbito | (DM) | Orchestrator |
| contabile | contabile | <THREAD_ID> | Accountant |
| lawrenzo | lawrenzo | <THREAD_ID> | Lawyer |
| groot | groot | <THREAD_ID> | Restaurant |
| wannabe | wannabe | (check dir) | Media |
| designbro | designbro | <THREAD_ID> | Designer |
| el-froggo | el-froggo | <THREAD_ID> | Trader |

## Sending a Message to Another Bot

### Method 1: send_message tool (preferred)
```
send_message(target="telegram:<CHAT_ID>:<THREAD_ID>", message="[FROM:GROOT] Nuovo evento sabato: Degustazione Barolo. Genera post social.")
```

### Method 2: hermes send CLI
```bash
HERMES_PROFILE=gribbito /usr/local/bin/hermes send --to telegram:<CHAT_ID>:<THREAD_ID> "message"
```

### Method 3: bot-send.sh helper
```bash
<HERMES_SHARED>/scripts/bot-send.sh gribbito contabile "F24 scade venerdì"
```
...
[FROM:DESIGNBRO] Grafica pronta: <HERMES_SHARED>/assets/menu-estate-2026.png. Pubblica.
...
See `<HERMES_SHARED>/registry/bots.json` for full connection map.
...
- Use Italian — all bots communicate in Italian
