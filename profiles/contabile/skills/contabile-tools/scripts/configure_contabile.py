...
from contabile_config import config_path, get_dotted, load, save, set_dotted, validate, profile_root
...
def cmd_apply_cron(_: argparse.Namespace) -> None:
    ...
    company = get_dotted(cfg, "company.name") or "Azienda"
    for job in data.get("jobs", []):
        if job.get("id") == "deadline_check":
            job["schedule"] = get_dotted(cfg, "cron.deadline_check") or "0 8 * * 1-5"
            job["message"] = f"📊 ContAIbile — scadenze fiscali {company}. Scrivi *scadenze*."
            job["target"] = f"telegram:{group}"
        if job.get("id") == "weekly_report":
            job["schedule"] = get_dotted(cfg, "cron.weekly_report") or "0 9 * * 1"
            job["message"] = f"📈 Report settimanale {company}. Scrivi *riepilogo*."
            job["target"] = f"telegram:{group}"
    CRON_JOBS.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps({"status": "applied"}, ensure_ascii=False))
